...
void print_pci_controller(struct pci_controller* ctrl)
{
	...
	pr_info("rtl8198c pcie DEBUG: io map base: 0x%08lX, io offset: 0x%08lX", ctrl->io_map_base, ctrl->io_offset);
	pr_info("rtl8198c pcie DEBUG: mem offset: 0x%08lX", ctrl->mem_offset);
	if (ctrl->io_resource) {
