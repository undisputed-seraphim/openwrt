...
static const struct rtl8198c_phy_param pcie0_phy_params[] __initconst = {
	{ 0x3, 0x7b31 }, { 0x6, 0xe258 },  { 0xF, 0x400F },
	{ 0xd, 0x1764 }, { 0x19, 0xFC70 },
};
static const struct rtl8198c_phy_param pcie1_phy_params[] __initconst = {
	{ 0x3, 0x3031 },
	{ 0x6, 0xe058 },
	{ 0xF, 0x400F },
	{ 0x19, 0xFC70 },
...
static inline struct rtl8198x_pci_controller *
get_rtl8198x_from_controller(struct pci_bus *pbus)
{
...
static void HostPCIe_SetPhyMdioWrite(uintptr_t mdioaddr, unsigned int regaddr,
				     unsigned short val)
{
...
#define PCIE_MDIO_RDWR_OFFSET (0)
	iowrite32(((regaddr & 0x1f) << PCIE_MDIO_REG_OFFSET) |
...
{
	iowrite32(0x01,
		  (void *)pcie_phy); // bit7: PHY reset=0 bit0: Enable LTSSM=1
	iowrite32(0x81,
		  (void *)pcie_phy); // bit7: PHY reset=1 bit0: Enable LTSSM=1
}
...
static int PCIE_reset_procedure(struct rtl8198x_pci_controller *ctrl,
				int portnum, int mdio_reset)
{
	pr_info("rtl8198c pcie: port = %d \n", portnum);

	//#ifdef CONFIG_RTL8198C
	REG32(CLK_MANAGE) = REG32(CLK_MANAGE) |
			    (1 << 12 | 1 << 13 | 1 << 14 | 1 << 16 | 1 << 19 |
			     1 << 20); //(1<<14);
	//#endif
...
#define SYS_PCIE_PHY1 (0xb8000000 + 0x54)
		...
		//#if defined(CONFIG_RTL8198C)
		REG32(0xb8000104) = (REG32(0xb8000104) & (~0x3 << 20)) |
				    (1 << 20); // PCIe MUX switch to PCIe reset

		{
			const uintptr_t mdioaddr = ctrl->host_mdio;
			const int phy40M = (ioread32((const void *)0xb8000008) &
					    (1 << 24)) >>
					   24;
			...
			if (phy40M) {
				...
				HostPCIe_SetPhyMdioWrite(mdioaddr, 0x6,
							 0xe258); // e2b8
				HostPCIe_SetPhyMdioWrite(mdioaddr, 0xF, 0x400F);
				HostPCIe_SetPhyMdioWrite(mdioaddr, 0xd,
							 0x1764); // e2b8
				HostPCIe_SetPhyMdioWrite(mdioaddr, 0x19,
							 0xFC70);
			} else {
...
{
	struct rtl8198x_pci_controller *rtl8198x_ctrl =
		get_rtl8198x_from_controller(bus);
...
{
	struct rtl8198x_pci_controller *ctrl =
		get_rtl8198x_from_controller(dev->bus);
	if (ctrl == NULL) {
...
{
	for (struct property *prop = node->properties; prop;
	     prop = prop->next) {
...
static int rtl8198c_parse_dt(struct rtl8198c_pcie *pcie)
{
