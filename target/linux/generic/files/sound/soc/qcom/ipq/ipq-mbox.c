...
#define CHN_DISABLED		0x00
#define CHN_ENABLED		0x01
#define CHN_STARTED		0x02
...
static inline u32 ipq_get_desc_size(void)
{
...
#define ipq_increment_desc(ptr, n) \
	((struct ipq_mbox_desc *)((u8 *)(ptr) + \
		(ipq_mbox_info.ipq_desc_size * (n))))

static struct ipq_mbox_desc *get_next(struct ipq_mbox_rt_dir_priv *rtdir,
				      struct ipq_mbox_desc *desc)
{
	...
	*ipq_mbox_get_last_played(unsigned int channel_id)
{
	...
	prev = ipq_increment_desc(
		mbox_rtime[index]->dir_priv[dir].dma_virt_head, ndescs - 1);
...
static irqreturn_t ipq_mbox_dma_irq(int irq, void *dev_id)
{
	...
	if (mask) {
		ack = IPQ_MBOX_IRQ_ACK(status, mask);
...
static int ipq_mbox_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		dev_err(&pdev->dev, "Error getting mbox resource\n");
		return -EINVAL;
	}

	irq = platform_get_irq(pdev, 0);
	if (irq < 0) {
		dev_err(&pdev->dev, "MBOX %d IRQ %d is not provided\n",
			id, irq);
		return irq;
	}
...
MODULE_DESCRIPTION("IPQ4019 Audio Mailbox Driver");
