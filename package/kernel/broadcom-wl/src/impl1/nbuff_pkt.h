...
#include <skb_compat.h>
#define PKTNEXT(osh, skb)                               ({ BCM_REFERENCE(osh); \
        skb_compat_next((struct sk_buff*)(skb));        })
#define PKTSETNEXT(osh, skb, skb_next)                  ({ BCM_REFERENCE(osh); \
        skb_compat_set_next((struct sk_buff*)(skb), (struct sk_buff*)(skb_next)); })
#else
#define PKTNEXT(osh, skb)                               ({ BCM_REFERENCE(osh); \
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)
#define PKTLINK(skb)                                                           \
        skb_compat_prev((struct sk_buff*)(skb))
#define PKTSETLINK(skb, x)                                                     \
        skb_compat_set_prev((struct sk_buff*)(skb), (struct sk_buff*)(x))
#else
#define PKTLINK(skb)                                                           \
...
#define PKTSHARED(skb)                                                         \
        skb_compat_cloned((struct sk_buff*)(skb))
#else
#define PKTSHARED(skb)                                                         \
